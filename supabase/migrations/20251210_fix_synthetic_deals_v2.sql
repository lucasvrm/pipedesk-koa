-- 20251210_fix_synthetic_deals_v2.sql

-- This migration patches the v2 synthetic data generator to ensure that deals
-- are created even in installations where the `master_deals` table does not
-- include a `company_id` column (or other newer columns like
-- `is_synthetic`/`deleted_at`).  Some older schemas lack these fields,
-- causing inserts to silently fail.  The revised function attempts the
-- full insert first and falls back to a minimal insert if a column is
-- undefined.  It also preserves the assignment of the deal owner in the
-- `deal_members` table.
...
BEGIN
    ...
    IF v_company_ids IS NOT NULL THEN
        FOR v_counter IN 1..p_deals_count LOOP
            v_owner_id := v_available_user_ids[floor(random()*array_length(v_available_user_ids, 1) + 1)];
            BEGIN
                -- Attempt full insert with company_id, is_synthetic and deleted_at columns
            ...
            EXCEPTION
                WHEN undefined_column THEN
                    -- If any of the above columns do not exist (e.g. company_id or is_synthetic),
                    -- fall back to a minimal insert using only the required fields.  Other
                    -- optional columns will use their defaults or remain NULL.
                    INSERT INTO master_deals (
                        client_name,
                        status,
                        created_by
