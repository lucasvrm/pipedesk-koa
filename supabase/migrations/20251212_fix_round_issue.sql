-- 20251212_fix_round_issue.sql

-- The previous version of generate_synthetic_data_v2 used round(double precision, integer),
-- which is not a valid signature in PostgreSQL. This migration corrects the fee
-- generation by casting the random result to numeric before rounding. No other
-- changes are introduced. Apply this migration to avoid runtime errors when
...
BEGIN
    ...
    IF v_company_ids IS NOT NULL THEN
        FOR v_counter IN 1..p_deals_count LOOP
            ...
            v_fee := round((random()*4 + 1)::numeric, 2); -- 1.00 to 5.00
            BEGIN
