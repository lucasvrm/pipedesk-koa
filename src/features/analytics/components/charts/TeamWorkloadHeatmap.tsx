...
export function TeamWorkloadHeatmap() {
  ...
  const workloadData = useMemo(() => {
    ...
    // Filtra apenas usuários relevantes (exclui clientes)
    ...
    return activeTeam.map(user => {
      ...
      // Cálculo simples de "Heat" (0 a 100)
      // Assumindo que 5 tracks ou 10 tarefas é "Full Capacity"
    ...
    }).sort((a, b) => b.score - a.score); // Mais ocupados primeiro
  ...
  return (
    <Card className="h-full">
      <CardHeader>
        ...
        <CardDescription>Intensidade de trabalho baseada em Deals e Tarefas ativas</CardDescription>
      </CardHeader>
      <CardContent>
        <div className="space-y-4">
          <div className="flex justify-between text-xs text-muted-foreground px-2">
            ...
            <div className="flex gap-8">
              <span className="w-16 text-center">Deals Ativos</span>
              ...
              <div key={item.user.id} className="flex items-center justify-between p-2 rounded-lg hover:bg-muted/50 transition-colors border">
                ...
                <div className="flex items-center gap-8">
                  <div className="w-16 text-center">
                    <span className="text-sm font-semibold">{item.tracks}</span>
                  </div>
                  
                  <div className="w-16 text-center">
                     <span className="text-sm font-semibold">{item.tasks}</span>
                  </div>
                      ...
                      <Tooltip>
                        <TooltipTrigger asChild>
                          <div className="h-2 w-full bg-slate-100 rounded-full overflow-hidden">
                            <div 
                              className={`h-full ${getHeatColor(item.score)} transition-all duration-500`} 
                              style={{ width: `${item.score}%` }}
                            />
                          </div>
                        </TooltipTrigger>
                        <TooltipContent>
                          <p>{item.score}% de Capacidade Estimada</p>
                        </TooltipContent>
                      </Tooltip>
                    </TooltipProvider>
                  </div>
