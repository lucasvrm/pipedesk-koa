      ...
      return (
        <div className="bg-popover border text-popover-foreground shadow-md rounded-lg p-3 text-xs">
          ...
          <div className="space-y-1">
            <p className="text-emerald-600 font-medium">Ponderado: {formatCurrency(payload[0].value)}</p>
            <p className="text-muted-foreground">Total Potencial: {formatCurrency(payload[1].value)}</p>
          </div>
        </div>
      );
    }
  ...
  return (
    <Card className="h-full">
      <CardHeader>
        ...
        <CardDescription>Volume Ponderado vs. Total Potencial</CardDescription>
      </CardHeader>
      <CardContent className="h-[400px]">
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={data} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
            <CartesianGrid strokeDasharray="3 3" vertical={false} className="stroke-muted/30" />
            <XAxis dataKey="month" className="text-xs capitalize" tickLine={false} />
            <YAxis tickFormatter={(val) => `R$${(val/1000000).toFixed(0)}M`} className="text-xs" axisLine={false} tickLine={false} />
            <Tooltip content={<CustomTooltip />} cursor={{ fill: 'transparent' }} />
            <Legend wrapperStyle={{ fontSize: '12px' }} />
            <Bar dataKey="ponderado" name="Ponderado" stackId="a" fill="#10b981" radius={[0, 0, 4, 4]} barSize={32} />
