...
export default function DocumentAutomationContent() {
  ...
  const [templateDialog, setTemplateDialog] = useState<{ open: boolean; item?: StructureTemplate | null }>({ open: false });
  const [docDialog, setDocDialog] = useState<{ open: boolean; item?: DocumentTypeConfig | null }>({ open: false });

  return (
    <>
      <SettingsSectionHeader
        title="Templates de Estrutura & Documentos"
        description="Configure padrões de pastas, nomes e gatilhos automáticos"
      >
        <Button variant="outline" size="sm" onClick={() => setTemplateDialog({ open: true })}>
          <Plus className="mr-2 h-4 w-4" /> Template
        </Button>
        <Button size="sm" onClick={() => setDocDialog({ open: true })}>
          <Plus className="mr-2 h-4 w-4" /> Tipo de Doc
        </Button>
      </SettingsSectionHeader>
      ...
      <Tabs defaultValue="templates" className="space-y-4">
        ...
        <TabsContent value="templates" className="space-y-3">
          {(templates ?? []).length === 0 ? (
            ...
            (templates ?? []).map((item) => (
              <div key={item.id} className="border rounded-lg px-4 py-3 flex items-start justify-between gap-4">
                <div className="space-y-1">
                  <div className="flex items-center gap-2">
                    <Badge variant="secondary" className="capitalize">{item.event_type.replaceAll('_', ' ')}</Badge>
                  ...
                  <p className="text-xs text-muted-foreground">
                    Documentos: {Array.isArray(item.documents_to_create) ? item.documents_to_create.length : 0} • {item.is_active ? 'Ativo' : 'Inativo'}
                  </p>
                </div>
                <div className="flex gap-2">
                  <Button variant="outline" size="sm" onClick={() => setTemplateDialog({ open: true, item })}>Editar</Button>
                  <Button variant="destructive" size="sm" onClick={() => deleteTemplate.mutateAsync(item.id!)}>Remover</Button>
                </div>
              </div>
            ))
          )}
        ...
        <TabsContent value="documents" className="space-y-3">
          {(documentTypes ?? []).length === 0 ? (
            <p className="text-sm text-muted-foreground text-center py-8">Nenhum tipo de documento cadastrado.</p>
          ) : (
              ...
              <div key={doc.id} className="border rounded-lg px-4 py-3 flex items-start justify-between gap-4">
                <div className="space-y-1">
                  <div className="flex items-center gap-2">
                    ...
                    <Badge variant="outline" className="capitalize">{doc.cardinality ?? 'single'}</Badge>
                    {doc.tags?.map((tag) => <Badge key={tag} variant="secondary">{tag}</Badge>)}
                  </div>
                  <p className="text-sm text-muted-foreground">{doc.folder_pattern}</p>
                  <p className="text-xs text-muted-foreground">
                    Nome: {doc.file_name_pattern || '—'} • Min: {doc.min_stage ?? '-'} • Obrig: {doc.required_stage ?? '-'}
                  </p>
                </div>
                <div className="flex gap-2">
                  <Button variant="outline" size="sm" onClick={() => setDocDialog({ open: true, item: doc })}>Editar</Button>
                  <Button variant="destructive" size="sm" onClick={() => deleteDocumentType.mutateAsync(doc.id!)}>Remover</Button>
                </div>
              </div>
            ))
          )}
      ...
      <p className="text-xs text-muted-foreground mt-6">
        Dica: use placeholders como {'{cliente}'} ou {'{stage}'} para formar caminhos e nomes.
      </p>
