...
**Changes:**
- Added import for `StandardPageLayout` from `@/components/layouts`
- Replaced `<div className="container max-w-4xl py-6 space-y-6">` wrapper with `<StandardPageLayout>`
- Removed width restrictions while maintaining exact CRUD functionality
- Page now uses full-width layout consistent with other admin settings pages

**Behavior preserved:**
- ✅ Logo upload/replace/remove functionality unchanged
- ✅ Favicon upload/replace/remove functionality unchanged
- ✅ Validation logic (file type, size) unchanged
- ✅ Error handling unchanged
- ✅ Hooks order maintained correctly (Rule 310)
...
- Dropped existing INSERT/UPDATE/DELETE policies for branding bucket
- Recreated policies to include `manager` role alongside `admin` and `analyst`
- Idempotent implementation using `DROP POLICY IF EXISTS`
...
**Alignment:**
- Route access: admin/manager (per App.tsx)
- Storage access: admin/manager/analyst
- ✅ No more 403 errors for managers attempting CRUD operations

---

### 3. **BRANDING_CUSTOMIZATION_IMPLEMENTATION.md**
**Changes:**
- Updated "Files Created" section to document both migrations
